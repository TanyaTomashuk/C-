...
Node* mergeLists(Node* first_node, Node* second_node){
    ...
    if ((first_node == NULL) && (second_node == NULL)){
        ...
        if (second_node == NULL){
            ...
            if ((first_node != NULL) && (first_node->value < second_node->value)){
                merged_node->value = first_node->value;
                merged_node->next = mergeLists(first_node->next, second_node);
            }
            else{
                merged_node->value = second_node->value;
                merged_node->next = mergeLists(first_node, second_node->next);
            }
        }
    }
    return merged_node;
